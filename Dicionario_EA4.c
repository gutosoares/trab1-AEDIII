...
NO_B B[TAM_B]; /* Arranjo de ponteiros para No */
...
void EA4_Insere_ou_Processa(struct No** R, Item I);
pNo EA4_Busca_B(Item I);
void EA4_Insere_B(pNo no);
void EA4_Inicializa_B();
void EA4_Imprime_B();
void EA4_Atualiza_B(int indice);
...
void DIC_Insere(Dicionario D, Item I)
{
	pNo n = EA4_Busca_B(I);
	if (n == NULL){
		EA4_Insere_ou_Processa( &(D->raiz), I );
	}else {
...
void EA4_Insere_ou_Processa(struct No** R, Item I)
{
	...
	if( Menor( Get_Chave( I ), Get_Chave( r->I ) ) )
	{
		EA4_Insere_ou_Processa( &(r->esq), I );
		return;
	}
	...
	if( Menor( Get_Chave( r->I ), Get_Chave( I ) ) )
	{
		EA4_Insere_ou_Processa( &(r->dir), I );
		return;
	}
...
pNo EA4_Busca_B(Item I){
	...
	while (i < TAM_B){
		...
		if (Igual(Get_Chave(I), Get_Chave((B[i].no)->I))){
...
/* Código respanvel em dar uma segunda chance
 para a chave analisada */
void EA4_Segunda_Chance()
{
	NO_B aux;
	int i;
...
void EA4_Imprime_B(){
	...
	int i;
	for (i = 0; i < TAM_B; i++){
		if (B[i].no != NULL) Escreve(B[i].no->I);
	}
	printf("\n");
}
...
/* Inserção das 10 palavras mais repitidas */
