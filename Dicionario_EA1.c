...
int Insere_ou_Processa_EA1(struct No** R, Item I);
...
void DIC_Insere(Dicionario D, Item I)
{
	...
	Insere_ou_Processa_EA1( &(D->raiz), I );
}
...
referente a estrategia de aceleração 1.
*/
void Insere_ou_Processa(struct No** R, Item I)
{
	struct No *r = *R;
...
int Insere_ou_Processa_EA1(struct No** R, Item I)
{
	...
	if( Menor( Get_Chave( I ), Get_Chave( r->I ) ) )
	{
		int a = Insere_ou_Processa_EA1( &(r->esq), I );
	...
	if( Menor( Get_Chave( r->I ), Get_Chave( I ) ) )
	{
		int a = Insere_ou_Processa_EA1( &(r->dir), I );
